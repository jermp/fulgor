...
template <typename T>
FastxParser<T>::FastxParser(std::vector<std::string> files, uint32_t numConsumers,
                            uint32_t numParsers, uint32_t chunkSize)
    : FastxParser(files, {}, numConsumers, numParsers, chunkSize) {}

template <typename T>
FastxParser<T>::FastxParser(std::vector<std::string> files, std::vector<std::string> files2,
                            uint32_t numConsumers, uint32_t numParsers, uint32_t chunkSize)
    : inputStreams_(files), inputStreams2_(files2), numParsing_(0), blockSize_(chunkSize) {
    ...
    readQueue_ =
        moodycamel::ConcurrentQueue<std::unique_ptr<ReadChunk<T>>>(4 * numConsumers, numParsers, 0);
    ...
    for (size_t i = 0; i < files.size(); ++i) { workQueue_.enqueue(i); }
...
FastxParser<T>::~FastxParser() {
    if (isActive_ or numParsing_ > 0) {
        ...
        std::cerr << "\n\nEncountered FastxParser destructor while parser was still marked active "
                     "(or while parsing threads were still active). "
...
bool FastxParser<T>::stop() {
    bool ret{false};
    if (isActive_) {
        for (auto& t : parsingThreads_) { t->join(); }
        isActive_ = false;
        for (auto& res : threadResults_) {
            if (res == -3) {
                ...
                std::stringstream ss;
                ss << "Error reading from the FASTA/Q stream. Minimum return code for left and "
                      "right read was ("
        ...
        std::cerr
            << "stop() was called on a FastxParser that was not marked active. Did you remember "
...
template <typename T>
int parse_reads(std::vector<std::string>& inputStreams, std::atomic<uint32_t>& numParsing,
    ...
    while (workQueue.try_dequeue(fn)) {
        ...
        while (!seqContainerQueue_.try_dequeue(*cCont, local)) {
            ...
            // Think of a way to do this that wouldn't be loud (or would allow a user-definable
            // logging mechanism) std::cerr << "couldn't dequeue read chunk\n";
        }
...
template <typename T>
int parse_read_pairs(std::vector<std::string>& inputStreams,
    ...
    if (numParsing_ == 0) {
        ...
        for (size_t i = 0; i < numParsers_; ++i) {
            ...
            parsingThreads_.emplace_back(new std::thread([this, i]() {
                this->threadResults_[i] =
                    parse_reads(this->inputStreams_, this->numParsing_,
...
template <>
bool FastxParser<ReadQualPair>::start() {
    if (numParsing_ == 0) {
        isActive_ = true;
        // Some basic checking to ensure the read files look "sane".
        if (inputStreams_.size() != inputStreams2_.size()) {
...
bool FastxParser<T>::refill(ReadGroup<T>& seqs) {
    ...
    while (numParsing_ > 0) {
        if (readQueue_.try_dequeue(seqs.consumerToken(), seqs.chunkPtr())) { return true; }
        fastx_parser::thread_utils::backoffOrYield(curMaxDelay);
    }
