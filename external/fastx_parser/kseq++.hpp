...
public:
    ...
    KStream(TFile f_, TFunc func_, format::Format fmt_,
            std::make_unsigned_t<size_type> bs_ = DEFAULT_BUFSIZE, close_type cfunc_ = nullptr)
    ...
    KStream(TFile f_, TFunc func_, spec_type, std::make_unsigned_t<size_type> bs_,
    ...
    KStream(TFile f_, TFunc func_, spec_type, close_type cfunc_)
        : KStream(std::move(f_), std::move(func_), mode::out, DEFAULT_FORMAT, DEFAULT_BUFSIZE,
                  cfunc_) {}
    ...
    /* Low-level methods */
    inline bool puts(std::string const& s, bool wrap = false) noexcept {
        ...
        while (cursor != s.size()) {
            ...
            if (wrap) len = std::min(len, this->wraplen - cursor % this->wraplen);
...
private:
    ...
    inline void worker_start() { this->worker = std::thread(&KStream::writer, this); }
};
...
public:
    ...
    inline bool getuntil(char_type delimiter, std::string& str, char_type* dret,
                         bool append = false)  // ks_getuntil
        noexcept {
        char_type c;
        bool gotany = false;
        if (dret) *dret = 0;
...
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, TSpec> make_kstream(TFile&& file,
                                                                             TFunc&& func, TSpec,
                                                                             Args&&... args) {
...
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::In_> make_ikstream(TFile&& file,
                                                                                  TFunc&& func,
                                                                                  Args&&... args) {
...
template <typename TFile, typename TFunc, typename... Args>
inline KStream<std::decay_t<TFile>, std::decay_t<TFunc>, mode::Out_> make_okstream(TFile&& file,
                                                                                   TFunc&& func,
                                                                                   Args&&... args) {
