...
void build_reference_sketches(index_type const& index,
    ...
    const uint64_t num_ones_per_thread = num_color_classes / num_threads;

    {
        uint64_t pop_count = 0;
        uint64_t prev_pos = 0;
        uint64_t end = num_ones_per_thread;
        pthash::bit_vector::unary_iterator unary_it(u2c);
        slice s;
        s.begin = prev_pos;
        s.color_id_begin = pop_count;
        s.color_id_end = end;
        for (uint64_t color_id = 0; color_id != num_color_classes; ++color_id) {
            uint64_t curr_pos = pop_count != num_ones ? unary_it.next() : (u2c.size() - 1);
            pop_count += 1;
            ...
            if (pop_count == end) {
                std::cout << "slice: begin=" << s.begin << "; color_ids[" << s.color_id_begin << ","
                          << s.color_id_end << ")" << std::endl;
                slices.push_back(s);
                s.begin = prev_pos;
                s.color_id_begin = pop_count;
                if (slices.size() == num_threads - 1) {
                    end = num_color_classes;
                } else {
                    end += num_ones_per_thread;
                }
                s.color_id_end = end;
            }
        }
        assert(slices.size() == num_threads);
    }

    auto exe = [&](uint64_t thread_id) {
        assert(thread_id < slices.size());
        auto& sketches = thread_sketches[thread_id];
        auto s = slices[thread_id];
        uint64_t pop_count = 0;
