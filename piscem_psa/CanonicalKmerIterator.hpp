...
public:
    ...
    CanonicalKmerIterator()
        : s_(), p_(), /*km_(), pos_(),*/ invalid_(true), lastinvalid_(-1), k_(CanonicalKmer::k()) {}
    CanonicalKmerIterator(const std::string& s)
        : s_(s)
        , p_()
        , /*km_(), pos_(),*/ invalid_(false)
        , lastinvalid_(-1)
        , k_(CanonicalKmer::k()) {
        find_next(-1, -1);
    }
    CanonicalKmerIterator(const CanonicalKmerIterator& o)
        : s_(o.s_)
        , p_(o.p_)
        , /*km_(o.km_), pos_(o.pos_),*/ invalid_(o.invalid_)
...
public:
    inline std::string_view seq() { return s_; }
    // use:  ++iter;
    // pre:
