...
/*
    from:
   https://github.com/jermp/sshash/blob/ce704aa8a7c28a2976ef86742861af8124e50601/include/util.hpp
...
*/
template <bool align>
[[maybe_unused]] static uint64_t word_reverse_complement(uint64_t x, uint64_t k) {
    assert(k <= 32);
...
static constexpr char revCodes[4] = {'A', 'C', 'T', 'G'};
/**
...
static decltype(codes[0]) codeForChar(char c) { return codes[static_cast<uint8_t>(c)]; }
static char charForCode(int i) { return revCodes[i]; }
static decltype(complements[0]) complement(char c) { return complements[static_cast<uint8_t>(c)]; }

// works for A, C, T, G
static int complement(int i) { return (0x3 & ((0x3 & i) + 2)); }
static bool isValidNuc(int i) { return i >= 0; }
static bool isValidNuc(char c) { return isValidNuc(codeForChar(c)); }
[[maybe_unused]] static bool notValidNuc(int i) { return !isValidNuc(i); }
[[maybe_unused]] static bool notValidNuc(char c) { return !isValidNuc(c); }
...
static const constexpr uint64_t maskTable[] = {
    bitmask<uint64_t>(0),  bitmask<uint64_t>(2),  bitmask<uint64_t>(4),  bitmask<uint64_t>(6),
    bitmask<uint64_t>(8),  bitmask<uint64_t>(10), bitmask<uint64_t>(12), bitmask<uint64_t>(14),
    bitmask<uint64_t>(16), bitmask<uint64_t>(18), bitmask<uint64_t>(20), bitmask<uint64_t>(22),
    bitmask<uint64_t>(24), bitmask<uint64_t>(26), bitmask<uint64_t>(28), bitmask<uint64_t>(30),
    bitmask<uint64_t>(32), bitmask<uint64_t>(34), bitmask<uint64_t>(36), bitmask<uint64_t>(38),
    bitmask<uint64_t>(40), bitmask<uint64_t>(42), bitmask<uint64_t>(44), bitmask<uint64_t>(46),
    bitmask<uint64_t>(48), bitmask<uint64_t>(50), bitmask<uint64_t>(52), bitmask<uint64_t>(54),
    bitmask<uint64_t>(56), bitmask<uint64_t>(58), bitmask<uint64_t>(60), bitmask<uint64_t>(62)};
 ...
 **/
[[maybe_unused]] static bool encodeBinary(char c, int64_t& code) {
 ...
 **/
[[maybe_unused]] static uint64_t charToBitsGATB(char c) {
...
static void reverseComplement(const std::string& seq, std::string& readWork) {
    ...
    if (start == end) { readWork[start] = (char)rc_table[(int8_t)seq[start]]; }
}
...
[[maybe_unused]] static std::string stringRevComp(const std::string& seq) {
    return reverseComplement(seq);
}

// Some template magic to detect if a template type has a ``length()'' function.
template <typename...>
using combinelib_void_t = void;

template <typename, typename = void>
struct has_length : public std::false_type {};

template <typename T>
struct has_length<T, combinelib_void_t<decltype(T().length())>> : public std::true_type {};
 ...
 **/
template <uint64_t K, uint64_t CID = 0>
class Kmer {
    static_assert(K <= 32, "Currently, the Kmer class can only represent k-mers of size <= 32");
...
public:
    ...
    bool fromChars(IterT iter) {
        ...
        for (int32_t w = 0; w < toConsume; ++w) {
            ...
            for (; remK > 0 and shift >= 0; ++iter, --remK, shift -= 2) {
                ...
                if (!encodeBinary(*iter, code)) return false;
                currWord |= (code << shift);
            }
        }
        return success;
    ...
    void rc() { data_[0] = word_reverse_complement<true>(data_[0], k_); }

    Kmer<K, CID> getRC() const {
        Kmer<K, CID> nk;
        nk.data_[0] = word_reverse_complement<true>(data_[0], k_);
        return nk;
    }

    void canonicalize() {
        auto wrc = word_reverse_complement<true>(data_[0], k_);
