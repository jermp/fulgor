...
void intersect_uncompressed(std::vector<color_info>& iterators, uint32_t num_colors, std::vector<uint32_t>& colors) {
    assert(colors.empty());
    ...
    while (candidate < num_colors) {
        for (; i != iterators.size(); ++i) {
            next_geq(iterators[i], candidate, num_colors);
...
void do_intersection(index_type& index, size_t num_threads_in, const std::string& query_filename, std::ofstream& output, std::atomic<uint64_t>& num_mapped_reads) {
    ...
    for (size_t i = 0; i < num_threads_in; ++i) {
        workers.push_back(
                std::thread([&index, &q, &done, &in_flight, &ofmut, &output, &num_mapped_reads, i]() -> void {
